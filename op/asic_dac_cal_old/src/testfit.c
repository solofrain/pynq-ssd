...
int cal_x[5]={0,15,31,47,63};
double centers[5][4][640];
float factor[4][640];
double mx=0.0;
double mn=0.0;
float dishi=3.0;
...
void scan_levels(){
/* scan external DACs, setting DAC valaues to x[i] and acquiring 
   intensities at each point. Data saved to file scan_data.dat. */
...
int save_factors(void){
FILE *fl;
int i,j;

	fl=fopen("factors.dat","w");
	printf("fl=%i    %s\n",fl,strerror(errno));
	if(fl != NULL){
	 for(i=0;i<4;i++){
	   for(j=0;j<NCH;j++){
		fprintf(fl,"%g ",factor[i][j]);
		printf("Factor(%i,%i)=%g\n",i,j,factor[i][j]);
	   }
	  }
	}
	else{
	  printf("File error! Factors not saved\n"); 
	}
...
void linfit(double *x, double *y, int n, double *p){
int i;
double sy,sx2,sx,sxy,s2x;
...
p[0]=(sy*sx2-sx*sxy)/(n*sx2-s2x);
p[1]=(n*sxy-sx*sy)/(n*sx2-s2x);
}


void Correct(){
 
  /* Calculates slopes of on-chip DACs from 5 values of 
     DAC settings vs fitted threshold position..
*/

double slopes[4][640],centers[5][4][640];
double xl[5],yl[5],pl[2];
int i,j,k,n;
